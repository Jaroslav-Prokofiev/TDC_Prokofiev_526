...
def encode():
    input_file = "Photo/5_3.bmp"
    output_file = "Results/Results.asf"
    ...
    with open("results_jpeg.txt", "a", encoding="utf-8") as file:
        print('Дані для сильнотекстурного зображення (таблиця квантування №2)\n', file=file)
...
def decoder():
    dc, ac, tables, blocks_count = read_image_file("Results/Results.asf")
    block_side = 8
    ...
    filename = "Results/decoded_image_5_3_2.jpg"
    image.save(filename)
    size_jpeg = os.path.getsize(filename)
    width, height = image.size
    size_ishodnogo = os.path.getsize("Photo/5_3.bmp")
    ratio = size_ishodnogo / size_jpeg

    with open("results_jpeg.txt", "a", encoding="utf-8") as file:
        print('Розмір файла JPEG: {} байт'.format(size_jpeg), file=file)
        print(f'Розмір зображення JPEG: {width}x{height}', file=file)
        print('Коефіцієнт стиснення= {:.2f}'.format(ratio), file=file)
        print('\n', file=file)
