...
def encode_ac(uniq_chars, probabilitys, alphabet_size, sequence):
    ...
    point = (low + high) / 2
    size_cod = math.ceil(math.log((1 / ((high - low) + 0.000000000000000001)), 2) + 1)
...
def encode_ch(uniq_chars, probabilitys, sequence):
    ...
    if 1 in probability and len(set(probability)) == 1:
        ...
        encode = ""
        for c in sequence:
            encode += [symbol_code[i][1] for i in rangegit add LosslessСompression(len(alphabet)) if symbol_code[i][0] == c][0]
...
with open("results_AC_CH.txt", "w", encoding="utf-8") as result_file:
    for sequence in original_sequences:
        ...
        result_file.write("______Арифметичне кодування______\n")
        result_file.write(f"Дані закодованої АС послідовності: {encoded_data_ac}\n")
        result_file.write(f"Закодована АС послідовність: {encoded_sequence_ac}\n")
        result_file.write(f"Значення bps при кодуванні АС : {bps_ac}\n")
        result_file.write(f"Декодована АС послідовність: {decoded_sequence_ac}\n")
        result_file.write("\n")
        result_file.write("______Кодування Хаффмана______\n")
        result_file.write("Алфавіт    Код символу\n")
        for item in encoded_data_ch[1]:
            result_file.write(f"{item[0]}          {item[1]}\n")
        result_file.write(f"Дані закодованої НС послідовності: {encoded_data_ch}\n")
        result_file.write(f"Закодована НС послідовність: {encoded_sequence_ch}\n")
        result_file.write(f"Значення bps при кодуванні НС : {bps_ch:.1f}\n")
        result_file.write(f"Декодована НС послідовність: {decoded_sequence_ch}\n")
        result_file.write("\n")
        result_file.write("////////////////////////////////////////////////////////////////////////////\n")
...
rows = ['Послідовність 1', 'Послідовність 2', 'Послідовність 3', 'Послідовність 4', 'Послідовність 5', 'Послідовність 6', 'Послідовність 7', 'Послідовність 8']
...
plt.savefig('Результати стиснення методами АС та СН.png', dpi=600, bbox_inches='tight')
plt.show()
